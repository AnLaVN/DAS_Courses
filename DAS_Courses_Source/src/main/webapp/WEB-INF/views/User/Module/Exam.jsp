...
<body>
	...
	<div class="m-0 mx-md-5 my-4 p-0 row">
		...
		<div class="col-12 col-lg-8 m-0 p-0">
			<form class="needs-validation" name="ExamForm" novalidate
				ng-init="Validation()" action="#" method="post">
					...
					<c:forEach var="cauhoi" items="${Course.cauhois}" varStatus="loop">
						...
						<div class="accordion-item">
							<h2 class="accordion-header">
								<button
									class="accordion-button collapsed${loop.index==0?'':' collapsed'} bg-opacity-75 fw-bold"
									type="button" data-bs-toggle="collapse"
									data-bs-target="#Sentence${loop.index}">
									<spring:message code="user.course.exam.question" />
									${loop.index+1}:
								</button>
							</h2>
							<div id="Sentence${loop.index}"
								class="accordion-collapse collapse${loop.index==0?' show':''}"
								data-bs-parent="#ExamSentencesDelete">
									...
									<div class="col-12 col-md-6 my-2">
										<div class="form-check">
											<input class="form-check-input" type="radio"
												name="AnswerOfSentence${loop.index}" value="A"
												id="AnswerAOfSentence${loop.index}" required> <label
												class="form-check-label"
												for="AnswerAOfSentence${loop.index}">A.
												${cauhoi.dapana}</label>
										</div>
									</div>
									<div class="col-12 col-md-6 my-2">
										<div class="form-check">
											<input class="form-check-input" type="radio"
												name="AnswerOfSentence${loop.index}" value="B"
												id="AnswerBOfSentence${loop.index}" required> <label
												class="form-check-label"
												for="AnswerBOfSentence${loop.index}">B.
												${cauhoi.dapanb}</label>
										</div>
									</div>
									<div class="col-12 col-md-6 my-2">
										<div class="form-check">
											<input class="form-check-input" type="radio"
												name="AnswerOfSentence${loop.index}" value="C"
												id="AnswerCOfSentence${loop.index}" required> <label
												class="form-check-label"
												for="AnswerCOfSentence${loop.index}">C.
												${cauhoi.dapanc}</label>
										</div>
									</div>
									<div class="col-12 col-md-6 my-2">
										<div class="form-check">
											<input class="form-check-input" type="radio"
												name="AnswerOfSentence${loop.index}" value="D"
												id="AnswerDOfSentence${loop.index}" required> <label
												class="form-check-label"
												for="AnswerDOfSentence${loop.index}">D.
												${cauhoi.dapand}</label>
										</div>
									</div>
				...
				<button type="submit"
					class="btn btn-lg btn-primary my-4 w-100 fw-bolder fs-5">
					<c:choose>
						<c:when test="${Mark > 0}">
							<spring:message code="user.course.exam.retest" />
						</c:when>
						<c:otherwise>
							<spring:message code="user.course.exam.finish" />
						</c:otherwise>
					</c:choose>
				</button>
			</form>
